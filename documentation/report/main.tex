...
\begin{flushleft}
\begin{abstract}
    O presente relatório descreve o processo de engenharia de software aplicado ao desenvolvimento de um sistema de gestão para uma oficina de reparação de trotinetes elétricas. Este projeto insere-se num contexto académico que visa não só a produção de uma solução funcional, mas também a exploração prática da utilização de Grandes Modelos de Linguagem (LLMs) como auxiliares no ciclo de vida de desenvolvimento de software.
    
    A solução concebida abrange todo o fluxo operacional da oficina, desde o registo inicial de clientes e equipamentos até à faturação final. O sistema permite a gestão integral de ordens de serviço, incluindo o controlo de diagnósticos, o registo de intervenções, a aplicação de peças e a contabilização de tempos de mão-de-obra. Adicionalmente, a aplicação assegura a gestão de stock de peças e disponibiliza relatórios técnicos e financeiros para apoio à tomada de decisão. O desenvolvimento seguiu metodologias de análise e desenho orientadas a objetos, com a implementação técnica suportada pela plataforma .NET e persistência de dados em SQL.
    
    O resultado final é um sistema funcional que responde aos requisitos de gestão de reparações, demonstrando a eficácia da integração de ferramentas de IA generativa na aceleração das etapas de conceção, codificação e documentação de sistemas de informação.

    \par \textbf{Área de Aplicação}: Engenharia de Software, Sistemas de Informação, Gestão de Oficinas.
    \par \textbf{Palavras-Chave}: LLM, .NET, C\#, SQL, UML, Gestão de Reparações, Bases de Dados Relacionais, Inteligência Artificial Generativa.
\end{abstract}
\end{flushleft}
...
\chapter{Introdução}
    \section{Contextualização}
        Nas últimas décadas, a Engenharia de Software tem vindo a enfrentar uma revolução significativa, impulsionada pelo aumento da complexidade dos sistemas, pela consolidação de arquiteturas distribuídas e, mais recentemente, pela integração de técnicas de Inteligência Artificial. O software deixou de ser um artefacto estático para se tornar num elemento central de sistemas dinâmicos, em evolução contínua e fortemente orientados a dados.
        
        Neste cenário, a emergência dos \textit{Large Language Models} (LLM) alterou profundamente os métodos de idealização, especificação, desenvolvimento e validação de software. Estes modelos atuam agora como agentes artificiais ativos, colaborando em tarefas que vão muito além da simples geração de código, abrangendo a Engenharia de Requisitos, a definição arquitetural e o suporte à decisão técnica.
        
        O presente trabalho prático insere-se neste novo paradigma, propondo uma abordagem onde o desenvolvimento de software é assistido sistematicamente por agentes artificiais ao longo de todo o ciclo de vida. Este modelo antecipa um futuro onde o engenheiro de software assume um papel mais estratégico e de governação, delegando tarefas cognitivamente intensivas ou repetitivas à Inteligência Artificial Generativa, visando ganhos de produtividade e redução do \textit{time-to-market}.
...
\renewcommand{\nompreamble}{Apresentar uma lista com todas as siglas e acrónimos utilizados...}

%% Definição das siglas (Exemplos)
\nomenclature[01]{\textbf{BD}}{Base de Dados}
\nomenclature[02]{DW}{Data Warehouse}
\nomenclature[03]{OLTP}{On-Line Analytical Processing}
\nomenclature[04]{LLM}{Large Language Model}

%% Este comando imprime a lista
\printnomenclature
